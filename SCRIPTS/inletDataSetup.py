...
class InletVelocityProfile:
    ...
    def create_time_dirs(self, timeDirList):
        for i in range(len(timeDirList) - 1):
            ...
            srcFile = os.path.join(self.baseDir, "constant/boundaryData/inlet/InletBCs_" + str(self.BPM) + "/U_" + timeDirList[i])
    ...
    def execute(self):
        if not os.path.exists(os.path.join(self.baseDir, "constant/boundaryData/inlet/InletBCs_" + str(self.BPM) + "/")):
            print("There is no inlet velocity profile data for BPM = " + str(self.BPM))
            return

        timeDirList = []
        subFile = os.listdir(os.path.join(self.baseDir, "constant/boundaryData/inlet/InletBCs_" + str(self.BPM) + "/"))
        subFile.sort()
