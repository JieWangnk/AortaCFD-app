...
class InletMapping:
    ...
    def write_out(self, inlet_data_file, inlet_name, time, vel, n_points, points, normal_vector, scale=1e0):
    ...
    def run(self, inlet_data_file, inlet_name, scale=1e0):
        # Read the velocity data from the csv file in the INLET folder
        times, velocities = self.read_csv_file(
            'constant/boundaryData/{}/{}'.format(inlet_name, inlet_data_file))
        n_points, points = self.read_points_file(
            'constant/boundaryData/{}/points'.format(inlet_name))
        ...
        self.write_out(inlet_data_file, inlet_name,
