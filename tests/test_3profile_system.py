...
class Test3ProfileSystem:
    ...
    def test_only_3_main_profiles_recommended(self):
        ...
        for profile in main_profiles:
            assert profile in NUMERICS_PROFILES, f"Main profile '{profile}' must exist in system"
    ...
    def test_no_old_monolithic_profiles(self):
        ...
        for old_profile in old_profiles:
            assert old_profile not in NUMERICS_PROFILES, f"Old monolithic profile '{old_profile}' should be removed"
...
class TestRobustProfile:
    ...
    def test_first_order_time_integration(self):
        ...
        assert self.profile["ddtSchemes"]["default"] == "Euler", "Robust must use Euler time integration"

    def test_first_order_convection(self):
        """Robust should use upwind (1st order, bounded)."""
        assert self.profile["divSchemes"]["div(phi,U)"] == "Gauss upwind", "Robust must use upwind convection for U"
        assert self.profile["divSchemes"]["div(phi,k)"] == "Gauss upwind", "Robust must use upwind for k"
        assert self.profile["divSchemes"]["div(phi,omega)"] == "Gauss upwind", "Robust must use upwind for omega"

    def test_courant_number(self):
        """Robust uses Co = 1.0 (first-order schemes are inherently stable)."""
        assert (
            self.profile["time_stepping"]["max_co"] == 1.0
    ...
    def test_many_correctors(self):
        ...
        assert (
            self.profile["solvers"]["PIMPLE"]["nOuterCorrectors"] == 25
        ), "Robust must use 25 max outer correctors (convergence-based exit)"
        assert self.profile["solvers"]["PIMPLE"]["nCorrectors"] == 3, "Robust must use 3 inner correctors"
        # Verify convergence-based early exit is configured
        assert (
            "outerCorrectorResidualControl" in self.profile["solvers"]["PIMPLE"]
    ...
    def test_moderate_relaxation_with_final_correction(self):
        ...
        assert (
            self.profile["solvers"]["relaxationFactors"]["equations"]["U"] == 0.7
        ), "Robust must use U relaxation = 0.7 (Windkessel compatible)"
        assert (
            self.profile["solvers"]["relaxationFactors"]["equations"]["UFinal"] == 1.0
        ), "Robust must use UFinal = 1.0 for full correction"
        assert (
            self.profile["solvers"]["relaxationFactors"]["fields"]["p"] == 0.3
        ), "Robust must use p relaxation = 0.3 (Windkessel compatible)"
        assert (
            self.profile["solvers"]["relaxationFactors"]["fields"]["pFinal"] == 1.0
    ...
    def test_relaxed_tolerances(self):
        ...
        assert self.profile["solvers"]["residualControl"]["p"] == 1e-4, "Robust must use residual tolerance 1e-4"
...
class TestStandardProfile:
    ...
    def test_second_order_time_integration(self):
        ...
        assert self.profile["ddtSchemes"]["default"] == "backward", "Standard must use backward time integration"

    def test_second_order_bounded_convection(self):
        """Standard should use limitedLinearV (2nd order TVD bounded)."""
        assert (
            "limitedLinearV" in self.profile["divSchemes"]["div(phi,U)"]
        ), "Standard must use limitedLinearV convection (2nd order TVD bounded)"
        assert (
            "limitedLinear" in self.profile["divSchemes"]["div(phi,k)"]
    ...
    def test_normal_courant_number(self):
        ...
        assert self.profile["time_stepping"]["max_co"] == 0.8, "Standard must use max_co = 0.8"

    def test_moderate_correctors(self):
        """Standard should use 10 max outer correctors with convergence-based early exit."""
        assert (
            self.profile["solvers"]["PIMPLE"]["nOuterCorrectors"] == 10
        ), "Standard must use 10 max outer correctors (convergence-based exit)"
        assert self.profile["solvers"]["PIMPLE"]["nCorrectors"] == 2, "Standard must use 2 inner correctors"
        # Verify convergence-based early exit is configured
        assert (
            "outerCorrectorResidualControl" in self.profile["solvers"]["PIMPLE"]
    ...
    def test_moderate_relaxation(self):
        ...
        assert (
            self.profile["solvers"]["relaxationFactors"]["equations"]["U"] == 0.8
        ), "Standard must use U relaxation = 0.8"
        assert (
            self.profile["solvers"]["relaxationFactors"]["fields"]["p"] == 0.5
        ), "Standard must use p relaxation = 0.5"

    def test_standard_tolerances(self):
        """Standard uses 1e-6 tolerances."""
        assert self.profile["solvers"]["residualControl"]["p"] == 1e-6, "Standard must use residual tolerance 1e-6"
...
class TestPreciseProfile:
    ...
    def test_crank_nicolson_time_integration(self):
        ...
        assert self.profile["ddtSchemes"]["default"] == "backward", "Precise must use backward time integration"
    ...
    def test_lust_convection(self):
        ...
        assert "LUST" in u_scheme, "Precise must use LUST convection (found: {})".format(u_scheme)
    ...
    def test_standard_gradient_limiting(self):
        ...
        assert "cellLimited" in grad_scheme and "1" in grad_scheme, "Precise must use cellLimited Gauss linear 1"
    ...
    def test_limited_laplacian(self):
        ...
        assert "limited" in laplacian, "Precise must use 'Gauss linear limited' Laplacian"

    def test_reduced_courant_for_accuracy(self):
        """Precise should use Co = 0.5 (smaller for temporal accuracy)."""
        assert self.profile["time_stepping"]["max_co"] == 0.5, "Precise must use max_co = 0.5 for temporal accuracy"
    ...
    def test_many_correctors(self):
        ...
        assert self.profile["solvers"]["PIMPLE"]["nCorrectors"] == 2, "Precise must use 2 inner correctors"
        # Verify convergence-based early exit is configured
        assert (
            "outerCorrectorResidualControl" in self.profile["solvers"]["PIMPLE"]
    ...
    def test_moderate_relaxation_with_final_correction(self):
        ...
        assert self.profile["solvers"]["relaxationFactors"]["fields"]["p"] == 0.5, "Precise must use p relaxation = 0.5"
        assert (
            self.profile["solvers"]["relaxationFactors"]["fields"]["pFinal"] == 1.0
    ...
    def test_tight_tolerances(self):
        ...
        assert self.profile["solvers"]["residualControl"]["p"] == 1e-8, "Precise must use residual tolerance 1e-8"
...
class TestUniversalPhysicsSupport:
    ...
    def test_laminar_support(self, profile_name):
        ...
        assert "div(phi,U)" in profile["divSchemes"], f"{profile_name} must support laminar (div(phi,U))"

    @pytest.mark.parametrize("profile_name", ["robust", "standard", "precise"])
    def test_rans_support(self, profile_name):
        """All profiles must support RANS (k, omega, epsilon schemes)."""
        profile = NUMERICS_PROFILES[profile_name]
        assert "div(phi,k)" in profile["divSchemes"], f"{profile_name} must support RANS (div(phi,k))"
        assert "div(phi,omega)" in profile["divSchemes"], f"{profile_name} must support RANS (div(phi,omega))"
        assert "div(phi,epsilon)" in profile["divSchemes"], f"{profile_name} must support RANS (div(phi,epsilon))"
    ...
    def test_les_support(self, profile_name):
        ...
        assert "div(phi,U)" in profile["divSchemes"], f"{profile_name} must have convection scheme for LES"
