...
app.config['UPLOAD_FOLDER'] = os.path.abspath(os.path.join(os.path.dirname(__file__), '../CAD'))
...
def run_sim():
    ...
    if request.method == 'POST':
        ...
        if case and profile:
            cmd = [
                'bash', '-c',
                'source /opt/openfoam8/etc/bashrc && python /home/mchi4jw4/GitHub/AortaCFD-app/app.py runAll --case {} --profile {}'.format(case, profile)
            ]
            try:
