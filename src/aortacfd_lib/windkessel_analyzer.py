...
class WindkesselAnalyzer:
    ...
    def _extract_from_boundary_fields(self) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        ...
        time_dirs = sorted([d for d in self.case_dir.iterdir()
    ...
    def _create_title_page(self, pdf: PdfPages, wk_params: Dict):
        ...
        fig.suptitle('Windkessel Boundary Condition Analysis',
...
"""
        ...
        ax.text(0.1, 0.8, info_text, transform=ax.transAxes,
        ...
        if wk_params:
            ...
            ax.text(0.1, 0.45, table_text, transform=ax.transAxes,
                    fontsize=9, verticalalignment='top', family='monospace')

        pdf.savefig(fig, bbox_inches='tight')
        plt.close()

    def _create_flow_rate_plots(self, pdf: PdfPages, times: np.ndarray,
    ...
    def _create_flow_distribution_plot(self, pdf: PdfPages,
                                      flow_rates: Dict[str, np.ndarray]):
        """Create pie chart of mean flow distribution."""
        fig, ax = plt.subplots(1, 1, figsize=(11, 8.5))
        fig.suptitle('Mean Flow Distribution Among Outlets',
        ...
        if mean_flows and sum(mean_flows.values()) > 1e-10:
            ...
            legend_labels = [f'{label}: {size:.2f} mL/s ({pct:.1f}%)'
    ...
    def _create_pressure_analysis_plots(self, pdf: PdfPages, times: np.ndarray):
        ...
        ax = fig.add_subplot(111)
        ax.axis('off')
        ax.text(0.5, 0.5, 'Pressure time-series data extraction not yet implemented.\n'
