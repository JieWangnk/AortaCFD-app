...
class SimulationReporter:
    ...
    def extract_mesh_stats(self) -> Dict:
        ...
        patch_section = re.search(
            r"Patch\s+Faces\s+Points.*?Surface topology\n(.*?)(?=\n\n|\nChecking)", content, re.DOTALL
        )
        if patch_section:
            patches = {}
            ...
            if postproc_dir.exists():
                ...
                for flow_dir in flow_dirs:
                    for data_file in flow_dir.glob("*/surfaceFieldValue.dat"):
                        try:
                            df = pd.read_csv(
                                data_file, sep=r"\s+", comment="#", names=["time", "flow_rate"], header=None
                            )
    ...
    def create_flow_rate_plot(
        self, inlet_data: pd.DataFrame, outlet_flows: Dict[str, pd.DataFrame], output_path: str
    ) -> str:
        ...
        ax.set_title("Flow Rate Comparison - Murray's Law Distribution", fontsize=14, fontweight="bold")
    ...
    def generate_html_report(self, output_path: str) -> str:
        ...
        html_content = self._generate_html_template(
            mesh_stats, transport_props, windkessel_data, solver_settings, plot_path
        )
        ...
        md_content = self._generate_markdown_template(
            mesh_stats, transport_props, windkessel_data, solver_settings, plot_path, inlet_data, outlet_flows
        )
...
"""
    ...
    def _generate_markdown_template(
        ...
        solver_settings: Dict,
        plot_path: Optional[str],
        inlet_data: Optional[pd.DataFrame],
        outlet_flows: Dict[str, pd.DataFrame],
    ) -> str:
        ...
        if plot_path:
            ...
            if inlet_data is not None:
                ...
                if outlet_flows:
                    flow_summary += "**Outlet Flow Distribution:**\n"
                    for outlet_name, outlet_data in outlet_flows.items():
