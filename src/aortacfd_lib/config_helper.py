...
class BoundaryConditionsHelper:
    ...
    def __init__(self):
        ...
        self.intelligent_defaults = {
            "geometry": {"rotation": True, "target_normal": [0, 0, 1], "scale_factor": 0.001},  # mm to m conversion
            "inlet": {"type": "TIMEVARYING", "data_type": "velocity", "orientation": "out"},
            "outlets": {"windkessel_settings": {"systolic_pressure": 120, "diastolic_pressure": 80}},
            "simulation_control": {"end_time": 0.1},  # Will be auto-calculated based on cycles
        }
    ...
    def _calculate_completeness(self, config: Dict[str, Any]) -> float:
        ...
        if config.get("geometry", {}).get("scale_factor"):
            present += 1
        if config.get("geometry", {}).get("refinement_level"):
            present += 1
        if config.get("geometry", {}).get("rotation") is not None:
            present += 1
        if config.get("geometry", {}).get("target_normal"):
            present += 1

        if config.get("inlet", {}).get("csv_file"):
            present += 1
        if config.get("inlet", {}).get("profile"):
            present += 1
        if config.get("inlet", {}).get("data_type"):
            present += 1
        if config.get("inlet", {}).get("type"):
            present += 1

        if config.get("outlets", {}).get("type"):
            present += 1
        if config.get("outlets", {}).get("windkessel_settings"):
            present += 1

        if config.get("simulation_control", {}).get("number_of_cycles"):
            present += 1
        if config.get("simulation_control", {}).get("end_time"):
            present += 1
        ...
        template = {
            ...
            "outlets": {
                ...
                "windkessel_settings": {
                    "systolic_pressure": 120,
                    "diastolic_pressure": 80,
                    "flow_split": {"outlet1": 0.25, "outlet2": 0.25, "outlet3": 0.25, "outlet4": 0.25},
                },
            },
            "simulation_control": {"number_of_cycles": 2, "end_time": 0.1},
        }
