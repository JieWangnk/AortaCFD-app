...
def check_executable(exe_name: str, version_flag: str = "--version") -> DependencyStatus:
    ...
    if exe_path:
        ...
        # Try to get version
        try:
            result = subprocess.run([exe_path, version_flag], capture_output=True, text=True, timeout=5)
...
def check_openfoam() -> DependencyStatus:
    ...
    if status.available:
        ...
        try:
            result = subprocess.run(["foamPostProcess", "-help"], capture_output=True, text=True, timeout=5)
