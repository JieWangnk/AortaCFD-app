...
class NumericsBuilder:
    ...
    def build(self, config: dict) -> dict:
        ...
        if missing_keys:
            raise ValueError(f"Configuration missing required keys: {missing_keys}. " f"Expected keys: {required_keys}")
    ...
    def _load_profile(self, profile_name: str) -> dict:
        ...
        if profile_name not in self.profiles:
            ...
            raise ValueError(f"Unknown numeric profile '{profile_name}'. " f"Available profiles: {available}")
    ...
    def _validate_numerics(self, numerics: dict, physics: dict, profile_name: str):
        ...
        if model == "rans" and u_scheme == "Gauss linear":
            warnings.warn(
                ...
                "────────────────────────\n"
                "Pure central differencing with RANS may cause numerical instability.\n\n"
                "Recommendation:\n"
                "  - Use 'Gauss limitedLinear 1' (bounded 2nd order)\n"
                "  - Use 'Gauss linearUpwind grad(U)' (upwind-biased)\n"
                "  - Use 'Gauss LUST grad(U)' (hybrid for publication)\n\n"
                "See: OpenFOAM User Guide, Section 4.4.2\n",
                UserWarning,
                stacklevel=2,
            )
        ...
        if profile_name == "precise":
            self.logger.info(
                ...
                "────────────────────────────\n"
                "LUST + Crank-Nicolson minimises numerical diffusion. Pair with:\n"
                "  1. Mesh independence: 3 levels, calculate GCI\n"
                "  2. Temporal convergence: halve Δt, verify < 1% change\n"
                "  3. Residuals: < 1e-8 for all variables\n"
                "  4. Compare to reference data if available\n"
                "  5. Document numerical choices in paper methods section\n"
            )
...
def recommend_numerics_profile(
    mesh_quality: dict, objective: str = "production", physics_model: str = "laminar"
) -> str:
    """
