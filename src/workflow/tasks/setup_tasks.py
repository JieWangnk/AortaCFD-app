...
class PrepareBoundaryDataTask(Task):
    ...
    def execute(self, context: dict) -> bool:
        ...
        try:
            ...
            run_command(
                config=self.config, command=["writeMeshObj"], case_directory=case_dir, log_filename="log.writeMeshObj"
            )
            ...
            formatter = EnhancedPointsFormatter(
                input_filename=temp_points_file, output_filename=os.path.join(case_dir, "points")
            )
            ...
            if inlet_type == "MRI":
                ...
                try:
                    ...
                    n_spatial_points = (
                        len(self._read_bare_points(mesh_pts_file)) if os.path.exists(mesh_pts_file) else 0
                    )
                    audit = InletAudit(
                        inlet_area_m2=0.0,
                ...
                if inlet_profile in ["wall_distance", "elliptical"]:
                    ...
                    self.log.info(
                        f"CONSTANT inlet with '{inlet_profile}' profile - generating non-uniform velocity data..."
                    )
                    ...
                    self.log.info("Non-uniform profile generated. Will use timeVaryingMappedFixedValue BC.")
                else:
                    self.log.info(
                        f"Inlet type is {inlet_type} with {inlet_profile} profile (uniform). Using fixedValue BC."
                    )
    ...
    def _calculate_constant_flowrate(self, inlet_config: dict, case_dir: str) -> float:
        ...
        if "flowrate" in inlet_config:
            ...
            self.log.info(
                f"CONSTANT inlet velocity: {velocity:.4f} m/s × area {inlet_area:.6e} m² = {flow_rate_m3s:.6e} m³/s"
            )
            return flow_rate_m3s

        else:
            self.log.error(
                "CONSTANT inlet with wall_distance profile requires 'flowrate', 'cardiac_output', or 'velocity'"
            )
    ...
    def _validate_inlet_csv(self, csv_path: str) -> bool:
        ...
        try:
            ...
            # Check shape
            if data.ndim < 2:
                raise ValueError("CSV must have at least 2 columns (time, value). Got 1D array.")
            if data.shape[1] < 2:
            ...
            self.log.info(
                f"CSV validation passed: {data.shape[0]} points, time range {times[0]:.4f} to {times[-1]:.4f}s"
            )
            return True
...
class GenerateControlDictTask(Task):
    ...
    def _calculate_purge_write(self, sim_controls: dict, control_dict: dict, cardiac_cycle: float) -> int:
        ...
        logger.info("Smart purgeWrite calculation:")
    ...
    def execute(self, context: dict) -> bool:
        ...
        try:
            report_path = report_gen.generate_full_report(
                config=self.config, geometry_info=geometry_info, mesh_info=mesh_info
            )
